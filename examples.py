...
def from_nothing(
    ...
    bw = BanditWare(
        feature_cols=feature_cols, save_dir=save_dir, model_choice=model_choice
    )

    print(
        "\tsuggested_hardware:",
        bw.suggest_hardware(prohibit_exploration=prohibit_exploration),
    )
...
def test_accuracy(data, feature_cols, model_choice, plot_predictions=False):
    ...
    bw.test_accuracy(
        model_choice=model_choice, plot_runtime_predictions=plot_predictions
    )
