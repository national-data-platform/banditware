...
def get_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Pre-process data for the integrated performance framework"
    )
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument(
        "--base_path",
        type=pathlib.Path,
        help="Path to the head folder where data is stored.",
    )
    group.add_argument(
        "--data_file", type=pathlib.Path, help="Path to the data csv file."
    )
    return parser
    ...
    for hardware_str, hardware_df in data.items():
        ...
        if match:
            ...
            if not isinstance(hardware_df, pd.DataFrame):
                print(
                    f"Error: Expected a DataFrame, but got {type(hardware_df)} for key {hardware_str}"
                )
                continue
...
def extend_df(
    df: pd.DataFrame, cpu_cores: int, mem_gb: int, ext_df: pd.DataFrame
) -> pd.DataFrame:
...
def preprocess(
    base_path: Union[str, None] = None, data_file: Union[str, None] = None
) -> pd.DataFrame:
