...
class BanditWare:
    ...
    def __init__(
        ...
        self._model_params: Dict[str, Any] = (
            {} if model_params is None else model_params.copy()
        )
        ...
        ), "`epsilon_start` must be a float between 0 and 1"
    ...
    def add_historical_data(
        ...
        if len(old_data) == 0:
            ...
            old_data["hardware"] = old_data["hardware"].apply(
    ...
    def predict_best_hardware(
        ...
        runtimes_by_hardware = {
            h: self._predict_runtime(model, features)
    ...
    def suggest_hardware(
        ...
        def get_suggestion(runtimes_by_hardware: Dict[int, float]) -> Tuple[int, int]:
            ...
            hardware_suggestion = HardwareManager.spec_from_hardware_idx(
                best_hardware_idx
            )
        ...
        found_matching_features = any(
            not np.isnan(runtime) for runtime in avg_runtimes_by_hardware.values()
        )
        ...
        if found_matching_features:
            num_hardwares_w_data = sum(
                not np.isnan(r) for r in avg_runtimes_by_hardware.values()
            )
    ...
    def test_accuracy(
        ...
        model = (
            ...
            else self._resolve_model_choice(model_choice)
        )
        model_params = (
            self._model_params if model_params is None else model_params.copy()
        )
        models_by_hardware: Dict[int, ModelInterface] = {
            h: model.create(**model_params) for h in self._hardwares
        }
        ...
        y_test_pred = np.array(
            [
                self._predict_runtime(models_by_hardware[h], x)
        ...
        if print_results:
            ...
            print(
                "Test Statistics:", acc_msg, rmse_msg, train_msg, test_msg, sep="\n\t"
            )

        # For debugging: see if the runtime predictions are reasonable on each hardware
        if plot_runtime_predictions:
            self.plot_runtime_predictions(
                data=test_data,
                model_instances=models_by_hardware,
    ...
    def change_application(
        ...
        new_save_dir: Union[str, pathlib.Path],
        feature_cols: Union[List[str], None] = None,
        model_choice: Union[Model, str, None] = None,
        model_params: Union[Dict[str, Any], None] = None,
    ):
        """
        ...
        self.reset_feature_cols(feature_cols)
        new_model_choice = (
            model_choice if model_choice is not None else Model.LINEAR_REGRESSION
        )
    ...
    def plot_runtime_predictions(
        ...
        for hardware_idx in self._hardwares:
            ...
            X = pd.DataFrame(
                data=data_portion[feature_cols], columns=feature_cols
            ).to_numpy()
        ...
        for hardware_idx in self._hardwares:
            ...
            unique_feature_sets = (
                data[feature_cols].drop_duplicates().itertuples(index=False, name=None)
            )
        ...
        df["mode"] = pd.Categorical(
            df["mode"], categories=["Actual", "Predicted"], ordered=True
        )
        ...
        if len(feature_cols) > 1:
            ...
            # found from testing on a laptop (arbitrary length)
            max_single_graph_title_len = 120
            max_subgraph_title_len = int(
                max_single_graph_title_len / len(model_instances)
            )
        ...
        x_order = [
            ", ".join(self._format_label(val) for val in feature_set)
            for feature_set in sorted(unique_features)
        ]
    ...
    def plot_runtime_spread(
        self,
        data: Union[pd.DataFrame, None] = None,
        feature_col: Union[str, None] = None,
        save: bool = False,
    ):
        """
        ...
        fig.update_traces(
            marker=dict(
                # Increase marker size (adjust as needed)
                size=8,
                line=dict(
    ...
    def _init_data(self, data=None) -> pd.DataFrame:
        ...
        # _model_instances does not exist until __init__ is done
    ...
    def _resolve_model_choice(
        self, model_choice: Union[str, Model, None] = None
    ) -> Model:
        """
        ...
        # If model_choice is a string, get the corresponding model and pass in hyperparameters
            ...
            try:
                ...
                raise ValueError(
                    f"model_choice must be one of the following:\n{all_models}"
                ) from e
    ...
    def _get_runtimes(
        self, features: np.ndarray, hardware: int, df: Union[pd.DataFrame, None] = None
    ) -> np.ndarray:
    ...
    def _sample_runtime(
        self, features: np.ndarray, hardware: int, df: pd.DataFrame
    ) -> float:
    ...
    def _get_hardware_avg_runtimes(
        self, features: np.ndarray, hardware: List[int], df: pd.DataFrame
    ) -> Dict[int, float]:
    ...
    def _get_best_hardware(
        self,
        runtimes_by_hardware: Dict[int, float],
        tolerance_ratio: Union[float, None] = DEFAULT_TOLERANCE_RATIO,
        tolerance_seconds: int = DEFAULT_TOLERANCE_SECONDS,
    ) -> int:
        """
        ...
        base_cpu_count, base_memory_gb = HardwareManager.spec_from_hardware_idx(
            fastest_hardware
        )
        ...
        for hardware_idx, runtime in runtimes_by_hardware.items():
            ...
            cpu_count, memory_gb = HardwareManager.spec_from_hardware(
                HardwareManager.get_hardware(hardware_idx)
            )
    ...
    def _split_data(self, data: pd.DataFrame, training_frac: float = 0.8):
        ...
        n_training_rows_left = ceil(training_frac * total_rows) - len(
            train_data_indices
        )
    ...
    def _best_hardwares_truth(
        self,
        unique_feature_sets: np.ndarray,
        full_data: pd.DataFrame,
        tolerance_ratio: Union[float, None] = None,
        tolerance_seconds: int = 0,
        void_missing_hardwares: bool = False,
    ) -> List[Union[int, None]]:
        """
        ...
        for features in unique_feature_sets:
            hardware_avg_runtimes = self._get_hardware_avg_runtimes(
                features, self._hardwares, df=full_data
            )
            missing_hardware_data = any(
                np.isnan(val) for val in hardware_avg_runtimes.values()
            )
        ...
        if warn_missing_data:
            print(
                "Warning: Some feature sets are not represented across all hardwares; Best hardware prediction may be inaccurate."
            )
        return best_hardwares

    def _get_models_accuracy(
        self,
        models: Dict[int, ModelInterface],
        unique_feature_sets: np.ndarray,
        ...
        truth_hardwares = self._best_hardwares_truth(
            unique_feature_sets,
            truth_dataset,
            tolerance_ratio=tolerance_ratio,
            tolerance_seconds=tolerance_seconds,
            void_missing_hardwares=ignore_incomplete_feature_rows,
        )
        ...
        for features in unique_feature_sets:
            ...
            predicted_hardware = self._get_best_hardware(
...
def get_parser_args():
    ...
    parser.add_argument(
        "--model", type=str, help="Type of model to use for runtime prediction"
    )
